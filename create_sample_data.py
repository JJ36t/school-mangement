...
def create_sample_data():
    ...
    if not User.objects.filter(username='admin').exists():
        ...
        print("مدير النظام موجود بالفعل")
    ...
    # Create subjects
    subjects_data = [
        {'name': 'الرياضيات', 'code': 'MATH101'},
        {'name': 'اللغة العربية', 'code': 'ARAB101'},
        {'name': 'العلوم', 'code': 'SCI101'},
        {'name': 'اللغة الإنجليزية', 'code': 'ENG101'},
    ]
    ...
    # Create classes
    classes_data = [
        {'name': 'الصف العاشر أ', 'class_level': 'tenth'},
        {'name': 'الصف العاشر ب', 'class_level': 'tenth'},
        {'name': 'الصف الحادي عشر أ', 'class_level': 'eleventh'},
        {'name': 'الصف الثاني عشر أ', 'class_level': 'twelfth'},
    ]
    ...
    for class_data in classes_data:
        class_obj, created = Class.objects.get_or_create(
            ...
            defaults={
                ...
                'class_location': f'قاعة {class_data["name"]}',
                'capacity': 30
            }
        )
    ...
    # Create students
    students_data = [
        {'first_name': 'سارة', 'last_name': 'أحمد', 'class_level': 'tenth'},
        {'first_name': 'علي', 'last_name': 'محمد', 'class_level': 'tenth'},
        {'first_name': 'مريم', 'last_name': 'حسن', 'class_level': 'eleventh'},
        {'first_name': 'يوسف', 'last_name': 'علي', 'class_level': 'eleventh'},
        {'first_name': 'زينب', 'last_name': 'أحمد', 'class_level': 'twelfth'},
        {'first_name': 'حسن', 'last_name': 'محمد', 'class_level': 'twelfth'},
    ]
    ...
    for i, student_data in enumerate(students_data):
        if not User.objects.filter(username=f'student{i+1}').exists():
            ...
            student = Student.objects.create(
                ...
                grade=student_data['class_level'].replace('tenth', '10').replace('eleventh', '11').replace('twelfth', '12'),
                section='أ',
                gender=random.choice(['male', 'female']),
